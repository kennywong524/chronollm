...
post_process.py
---------------
Post-processing script for conditional sorting experiments.

Supports both GPT-5 and Claude 3.7 experiment outputs.

Usage:
    python post_process.py --results-dir <path_to_results> [--output-dir <output_dir>]
"""
...
def analyze_filtering(results):
    ...
    for result in results:
        ...
        filtering_analysis.append({
            ...
            'extended_thinking': result.get('extended_thinking', None),
...
def create_summary_statistics(filtering_df, ordering_df):
    summary = []
    # Filtering accuracy by criterion and ET (if applicable)
    group_cols = ['criterion']
    if 'extended_thinking' in filtering_df.columns and filtering_df['extended_thinking'].notna().any():
        group_cols.append('extended_thinking')
    
    for group_key, group in filtering_df.groupby(group_cols):
        if isinstance(group_key, tuple):
            criterion = group_key[0]
            ext = group_key[1] if len(group_key) > 1 else None
        else:
            criterion = group_key
            ext = None
        
        condition_suffix = ('ET' if ext else 'NoET') if ext is not None else ''
        condition = f'{criterion}_{condition_suffix}_self_filtered' if condition_suffix else f'{criterion}_self_filtered'
    ...
    # Ordering summary by criterion, ET (if applicable), and task type
    order_group_cols = ['criterion', 'task_type']
    if 'extended_thinking' in ordering_df.columns and ordering_df['extended_thinking'].notna().any():
        order_group_cols.insert(1, 'extended_thinking')
    
    for group_key, group in ordering_df.groupby(order_group_cols):
        if isinstance(group_key, tuple):
            criterion = group_key[0]
            ext = group_key[1] if len(group_key) > 2 else None
            task_type = group_key[-1]
        else:
            criterion = group_key[0]
            ext = None
            task_type = group_key[1]
        ...
        condition_suffix = ('ET' if ext else 'NoET') if ext is not None else ''
        condition = f'{criterion}_{condition_suffix}_{task_type}' if condition_suffix else f'{criterion}_{task_type}'
...
def main():
    parser = argparse.ArgumentParser(description="Post-process conditional sorting experiment results")
