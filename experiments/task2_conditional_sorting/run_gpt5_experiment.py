"""
run_gpt5_experiment.py
----------------------
Run GPT-5 conditional sorting experiments.
...
def call_gpt(prompt, model, temperature, reasoning_effort=None):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if reasoning_effort and model.startswith("gpt-5") and model != "gpt-5-chat-latest":
...
def run_experiment(model, reasoning_effort=None):
    """Run the conditional sorting experiment."""
    # Determine output directory name
    if model == "gpt-5-chat-latest":
        model_dir = "gpt-5-chat-latest"
    elif reasoning_effort:
        model_dir = f"gpt-5-{reasoning_effort}"
    else:
        model_dir = model.replace("/", "_")
    
    OUT_DIR = BASE_OUT_DIR / model_dir
    ...
    # Match files based on reasoning effort if specified
    if reasoning_effort:
        existing_files = list(OUT_DIR.glob(f"*_gpt5_{reasoning_effort}_*.json"))
    else:
        existing_files = list(OUT_DIR.glob("*_gpt5_*.json"))
        # Exclude files with reasoning effort in name
        existing_files = [f for f in existing_files if not any(re_str in f.name for re_str in ["minimal", "low", "medium", "high"])]
    if existing_files:
    ...
    for criterion in CRITERIA:
        ...
        for trial in range(N_TRIALS):
            ...
            # Include reasoning effort in trial_id if specified
            if reasoning_effort:
                trial_id = f"{criterion_name}_gpt5_{reasoning_effort}_{trial:04d}"
            else:
                trial_id = f"{criterion_name}_gpt5_{trial:04d}"
            ...
            response_obj = call_gpt(prompt, model, TEMPERATURE, reasoning_effort)
            ...
            given_response_obj = call_gpt(given_prompt, model, TEMPERATURE, reasoning_effort)
            ...
            # Save each trial (include reasoning effort in filename if specified)
            if reasoning_effort:
                out_file = OUT_DIR / f"{criterion_name}_gpt5_{reasoning_effort}_{trial:04d}.json"
            else:
                out_file = OUT_DIR / f"{criterion_name}_gpt5_{trial:04d}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run GPT-5 conditional sorting experiment")
    parser.add_argument("--model", type=str, default="gpt-5", help="Model name")
    parser.add_argument("--reasoning-effort", type=str, default=None, choices=["minimal", "low", "medium", "high"], 
                       help="Reasoning effort level (only for gpt-5). Required for gpt-5 model.")
    ...
    # Validate reasoning effort for gpt-5
    if args.model == "gpt-5" and args.reasoning_effort is None:
        print("ERROR: --reasoning-effort is required when using gpt-5 model")
        print("Please specify: --reasoning-effort minimal|low|medium|high")
        sys.exit(1)
    
    run_experiment(args.model, args.reasoning_effort)
