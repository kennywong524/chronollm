...
----------------------
Run GPT-5 chronological ordering experiments with varying reasoning effort levels.
...
def call_gpt(prompt, model, temperature, reasoning_effort=None):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # Add reasoning_effort for GPT-5 models (except gpt-5-chat-latest)
...
def run_experiment(model, reasoning_effort=None, temperature=1.0):
    ...
    print(f"Model: {model}, Reasoning Effort: {reasoning_effort or 'N/A'}")
    ...
    for n_presidents in SAMPLE_SIZES:
        ...
        for i in range(trials_per_size[n_presidents]):
            ...
            try:
                ...
                # Get model response
                response_dict = call_gpt(prompt, model, temperature, reasoning_effort)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run GPT-5 chronological ordering experiment")
    parser.add_argument("--model", type=str, default="gpt-5", help="Model name (e.g., gpt-5, gpt-5-chat-latest)")
    parser.add_argument("--reasoning-effort", type=str, choices=["minimal", "low", "medium", "high"], 
                       help="Reasoning effort level (only for gpt-5)")
    ...
    run_experiment(args.model, args.reasoning_effort, args.temperature)
