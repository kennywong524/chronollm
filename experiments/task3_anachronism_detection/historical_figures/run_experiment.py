...
run_experiment.py
-----------------
Run the historical figures anachronism detection experiment.
...
# Configuration
GROUND_TRUTH_CSV = "data/historical_figures_ground_truth.csv"
BASE_OUT_DIR = Path("results/task3_anachronism_detection/historical_figures")
BATCH_SIZE = 20
...
def format_prompt(pairs):
    ...
    prompt = (
        ...
        f"Below are {BATCH_SIZE} statements about U.S. presidents and historical figures.\n"
        f"For each, respond with 'Possible' if the president could have received a letter from that figure during their presidency, or 'Not possible' if not.\n"
        "\n**IMPORTANT: Only consider chronological plausibility (whether their lifetimes overlapped), NOT logical plausibility.**\n"
        "For example, if a president and figure were both alive during the president's term, it's 'Possible' even if the figure was very young or old.\n"
        "A president could theoretically receive a letter from a toddler or elderly person if they were alive at the same time.\n"
        "\nGive NO explanation.\n"
        ...
        "\n**Example output:**\n"
        "George Washington Received a letter from Napoleon Bonaparte: Possible\n"
        "George Washington Received a letter from Albert Einstein: Not possible\n"
        "Thomas Jefferson Received a letter from Charles Dickens: Not possible\n"
        "Abraham Lincoln Received a letter from Queen Victoria: Possible\n"
        "Franklin D. Roosevelt Received a letter from Winston Churchill: Possible\n"
        "Barack Obama Received a letter from Steve Jobs: Possible\n"
...
def main():
    parser = argparse.ArgumentParser(description="Run historical figures anachronism detection experiment")
    ...
    true_pairs = df[df["possible"] == True][["president", "event", "first_possible_year", "last_possible_year"]].to_dict("records")
    false_pairs = df[df["possible"] == False][["president", "event", "first_possible_year", "last_possible_year"]].to_dict("records")
    ...
    for exp_type, n_true, n_false in EXPERIMENT_TYPES:
        ...
        for _ in range(BATCHES_PER_TYPE):
            try:
                ...
                out_file = OUT_DIR / f"historical_figures_batch_{batch_id:04d}.json"
