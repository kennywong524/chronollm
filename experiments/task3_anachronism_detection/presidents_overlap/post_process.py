...
post_process.py
---------------
Post-process presidents overlap experiment results.
...
def parse_model_output(model_output, event_list):
    """Parse the model's output and return a list of booleans (True for Yes, False for No)."""
    ...
    for event in event_list:
        ...
        if match is None:
            preds.append(np.nan)
            continue
        if re.search(r":\s*Yes\s*$", match, re.IGNORECASE):
            preds.append(True)
        elif re.search(r":\s*No\s*$", match, re.IGNORECASE):
            preds.append(False)
        else:
            preds.append(np.nan)
    return preds
...
def main():
    parser = argparse.ArgumentParser(description="Post-process presidents overlap experiment results")
    ...
    json_files = sorted(results_dir.glob("presidents_overlap_batch_*.json"))
    if not json_files:
    ...
    for f in tqdm(json_files, desc="Processing batches"):
        ...
        event_list = [pair['event'] for pair in d["pairs"]]
        ground_truth = [pair["ground_truth"] for pair in d["pairs"]]
        president_groups = [pair["president_group"] for pair in d["pairs"]]
        ...
        for i, event in enumerate(event_list):
            processed.append({
                ...
                "president_group": president_groups[i],
                "event": event,
