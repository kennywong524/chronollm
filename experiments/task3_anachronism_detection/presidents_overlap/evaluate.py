...
evaluate.py
-----------
Compute evaluation metrics for the presidents overlap experiment.
...
def evaluate_presidents_overlap(csv_path, output_path=None):
    ...
    # Remove duplicates based on president_group
    eval_df_unique = eval_df.drop_duplicates(subset=["president_group"], keep='first')
    ...
    rows = []
    for exp_type, g in eval_df_unique.groupby("experiment_type"):
        yt = g["ground_truth"].astype(bool)
        yp = g["model_prediction"].astype(bool)
        acc_ = (yt == yp).mean()
    ...
    if not metrics_df.empty:
        ...
        print("\nMacro-average across experiment_type:")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate presidents overlap anachronism detection results")
    parser.add_argument("--input", type=str, required=True, help="Path to pred_vs_truth.csv")
    parser.add_argument("--output", type=str, default=None, help="Output CSV path")
    
    args = parser.parse_args()
    evaluate_presidents_overlap(args.input, args.output)
